export default {
  en: {
    home: {
      developerTitle: 'Front-end Developer',
      javascriptTitle: 'Javascript Expert',
      messages: [
        'Likes penguins',
        'Loves Sakura Ayane',
        'Actually owns a Vita',
        'Reads 1 book per week',
        'Built this site',
        'Prefers spaces over tabs',
        'THE TRUTH IS OUT THERE',
        'Collects mangas',
        'Puts stickers on his laptop',
        'Rubik\'s cubes\' solver',
        'Knows how to juggle',
        'Hated the Angular2 RC fase',
        'Loves to share knowledge',
        'The bg video is from YuruYuri',
        'EMs are the superior css unit',
        'Loves to animate using css',
        'Shares side-project on github',
        'Needs to add a SW to the site',
        'Believes RWD is a must-have',
        'Using React since early 2015',
        'Fell in love with Vue.js',
        'WORDS, WORDS, WORDS.',
        'How you doin\'',
        'Currently A2 on Shadowverse'
      ]
    }
  },

  es: {
    home: {
      developerTitle: 'Desarrollador Front-end',
      javascriptTitle: 'Experto en JS',
      messages: [
        'Amante de los pinguinos',
        'Enamorado de Sakura Ayane',
        'En verdad compro un Vita',
        'Lee 1 libro por semana',
        'Autor de este sitio',
        'Prefiere espacios a Tabs',
        'LA VERDAD ESTA AHI FUERA',
        'Collecciona mangas',
        'Le pone stickers a su laptop',
        'Soluciona cubos de Rubik',
        'Sabe hacer malabares',
        'Odio la fade RC de Angular2',
        'Le encanta compartir info',
        'El video es de YuruYuri',
        'EM es la unidad superior',
        'Adora animar cosas con CSS',
        'Comparte proyectos en Github',
        'Ocupa agregar un SW al sitio',
        'Cree que RWD es obligatorio',
        'Usando react desde el 2015',
        'Me he enamorado de Vue.js',
        'CHISTE PERDIDO AL TRADUCIR',
        'Hola',
        'Actualmente A2 en Shadowverse'
      ]
    }
  }
}
